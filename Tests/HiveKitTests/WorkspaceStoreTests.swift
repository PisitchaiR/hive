...
final class WorkspaceStoreTests: XCTestCase {
    ...
    /// Two independent stores wired as each other's peers — models two hive
    ...
    func testCommandFinishedKeepsTerminalTitle() {
        ...
        // just before hive's OSC 133;D fires (hive's 133 hook runs last in
    ...
    func testAddTabRespectsTemplate() {
        ...
        XCTAssertEqual(engine(session).startedConfigs.first?.environment["HIVE_AGENT"], "claude")
    }
    ...
    func testApplyConversationIdWritesToCorrectSessionOnly() {
        ...
        // other. Same isolation we get in prod via HIVE_SURFACE_ID routing.
        let store = makeStore()
        let ws = store.addWorkspace(workingDirectory: projectA)
        let pane = firstPane(ws)
    ...
    func testAddTabPropagatesInitialPromptToSpawnedEngine() {
        ...
        XCTAssertEqual(cfg?.environment["HIVE_AGENT"], "claude -- 'explain this'")
    }
