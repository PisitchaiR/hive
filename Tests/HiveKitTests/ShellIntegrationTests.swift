...
/// real user config files. Self-heals on next hive launch but better
/// avoided: the writers are trivial, the content getters are the
/// load-bearing surface.
final class ShellIntegrationTests: XCTestCase {
    private static let stubHook = "/usr/local/bin/HiveHook"

    func testGeminiDefaultsExposesAllFourLifecycleEvents() throws {
        let object = HiveShellIntegration.geminiDefaultsObject(hookCmd: Self.stubHook)
    ...
    func testClaudeHooksObjectStaysWiredAfterRefactor() throws {
        let object = HiveShellIntegration.claudeHooksObject(hookCmd: Self.stubHook)
        let hooks = try XCTUnwrap(object["hooks"] as? [String: Any])
    ...
    func testBracketWrapperPassesThroughWhenSurfaceIdMissing() {
        let script = HiveShellIntegration.bracketWrapperScript(slug: "amp")

        XCTAssertTrue(script.contains("self_dir"), "must skip own dir on PATH walk")
        XCTAssertTrue(script.contains("\"$HIVE_HOOK_BIN\" amp running"))
        XCTAssertTrue(script.contains("\"$HIVE_HOOK_BIN\" amp ended"))
        XCTAssertTrue(script.contains("exec \"$real\" \"$@\""), "must passthrough when HIVE_SURFACE_ID is unset")
    }
    ...
    func testAntigravityWrapperGuardsAgainstIDEShim() {
        ...
        let script = HiveShellIntegration.antigravityWrapperScript
        ...
        XCTAssertTrue(script.contains("\"$HIVE_HOOK_BIN\" agy ended"), "must revert tab icon on shim-detection bail")
    ...
    func testAntigravityWrapperBracketsRunningAndEndedForRealCLI() {
        ...
        XCTAssertTrue(script.contains("\"$HIVE_HOOK_BIN\" agy running"))
        XCTAssertTrue(script.contains("exec \"$real\" \"$@\""), "must passthrough when HIVE_SURFACE_ID is unset")
    }

    func testOpencodePluginShellsOutToHookBinForBothEvents() {
        let body = HiveShellIntegration.opencodePluginScript
        ...
        XCTAssertTrue(body.contains("opencode"), "plugin must pass agent slug to HiveHook")
        XCTAssertTrue(body.contains("HIVE_SURFACE_ID"))
        XCTAssertTrue(body.contains("hive-managed-do-not-edit"), "plugin must carry the upgrade-safety marker")
    }

    func testEnvStatusBlockReportsLiveShellEnvironment() {
        let body = HiveShellIntegration.envStatusBlock

        XCTAssertTrue(body.contains("\"$HIVE_HOOK_BIN\" env"))
        ...
        XCTAssertTrue(body.contains("_HIVE_NODE_KEY_LAST"), "must memoize node version against path+NVM_BIN")
        XCTAssertTrue(body.contains("_HIVE_ENV_LAST"), "must skip the hive-hook IPC when env unchanged")
    }
    ...
    func testHookServerParsesShellEnvironmentPayload() throws {
        ...
        XCTAssertEqual(env["HIVE_NODE_VERSION"], "v20.1.0")
    }

    func testBackslashEscapeLeavesPlainPathUntouched() {
        XCTAssertEqual(HiveShellIntegration.backslashEscape("/Users/corey/file.txt"), "/Users/corey/file.txt")
    }

    func testBackslashEscapeEscapesSpaceAndQuoteAndDollar() {
        XCTAssertEqual(
            HiveShellIntegration.backslashEscape("/Users/corey/My Folder/don't $cost"),
    ...
    func testBackslashEscapePassesThroughNonAscii() {
        ...
        XCTAssertEqual(HiveShellIntegration.backslashEscape("/tmp/项目/🚀.md"), "/tmp/项目/🚀.md")
    }

    func testClaudeCustomSettingsObjectCarriesHooksAndEnv() throws {
        let object = HiveShellIntegration.claudeCustomSettingsObject(
        ...
        // file passed to that session, so hive's activity hooks have to be
    ...
    func testBackslashEscapeFallsBackToQuoteOnNewlineToAvoidLineContinuation() {
        ...
        // literal newline.
        let escaped = HiveShellIntegration.backslashEscape("/tmp/multi\nline/file.txt")
