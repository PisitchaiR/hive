...
struct TerminalSessionConfig {
    ...
    static func defaultShell() -> TerminalSessionConfig {
        let shell = ProcessInfo.processInfo.environment["SHELL"] ?? HiveShellIntegration.zshPath
...
protocol TerminalEngine: AnyObject {
    ...
    /// shell emits its own `user@host:dir` title — shows in hive's chrome.
    ...
    /// is a passive mirror — hive doesn't push the needle string itself.
