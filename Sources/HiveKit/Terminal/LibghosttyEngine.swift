...
final class LibghosttyApp {
    ...
    private init() {
        ...
        guard initResult == 0 else {
            NSLog("hive: ghostty_init failed (\(initResult))")
            return
        }
        ...
        var runtime = ghostty_runtime_config_s(
            ...
            wakeup_cb: hiveWakeupCb,
            action_cb: hiveActionCb,
            read_clipboard_cb: hiveReadClipboardCb,
            confirm_read_clipboard_cb: hiveConfirmReadClipboardCb,
            write_clipboard_cb: hiveWriteClipboardCb,
            close_surface_cb: hiveCloseSurfaceCb
        )

        let config = ghostty_config_new()
        // Config precedence, last-write wins:
        //   ghostty defaults → hive baselines → ~/.hive/settings.json
        // So user's hive settings beat both, and hive baselines fill in
...
private let hiveWakeupCb: ghostty_runtime_wakeup_cb = { _ in
...
private let hiveActionCb: ghostty_runtime_action_cb = { _, target, action in
    ...
    case GHOSTTY_ACTION_SHOW_CHILD_EXITED:
        // exit 0 → user typed `exit` / `logout`; auto-close the hive tab
...
private let hiveReadClipboardCb: ghostty_runtime_read_clipboard_cb = { _, _, _ in false }
private let hiveConfirmReadClipboardCb: ghostty_runtime_confirm_read_clipboard_cb = { _, _, _, _ in }
private let hiveWriteClipboardCb: ghostty_runtime_write_clipboard_cb = { _, kind, contents, count, _ in
...
private let hiveCloseSurfaceCb: ghostty_runtime_close_surface_cb = { _, _ in }
...
final class GhosttySurfaceView: NSView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        let escaped = urls.map { HiveShellIntegration.backslashEscape($0.path) }.joined(separator: " ")
        paste(escaped)
        return true
    }
    ...
    func createSurfaceIfReady() {
        ...
        guard let new else {
            NSLog("hive: ghostty_surface_new failed")
            return
        }
        surface = new
        pendingConfig = nil
    ...
    override func keyDown(with event: NSEvent) {
        ...
        // active mode instead of hive hard-coding CSI forever. If libghostty
        ...
        // Hive-specific functional keys with explicit byte behavior. Skipped
    ...
    /// are post-translation control sequences hive already encodes itself.
    ...
    /// Map hive's own functional-key policy to bytes. Returns `nil` for
