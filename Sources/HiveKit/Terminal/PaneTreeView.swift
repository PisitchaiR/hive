    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            if let active = pane.activeTab {
                if let fileURL = active.fileViewerURL {
                    FileContentViewer(url: fileURL)
                        .id(active.id)
                } else if let diffInfo = active.gitDiffInfo {
                    GitDiffViewer(info: diffInfo)
                        .id(active.id)
                } else {
                ...
                Rectangle().fill(Theme.chromeHairline).frame(height: 1)
                PaneStatusBar(session: active, store: store)
            } else {
                Color.clear
            }
        }
...
private struct PaneStatusBar: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            HoverableIconButton(
                ...
                HiveSettingsWindowController.show(storeProvider: { [weak store] in store })
            }
        }
    ...
    private var diffSegment: some View {
        ...
        if s.branch != nil, s.filesChanged > 0 {
            DiffToggleSegment(gitStatus: s, store: store)
        }
    }
}
...
/// Shared clickable shell for any `StatusSegment` that needs hover + active
/// background states. Callers provide `isActive` (e.g. panel open, popover
/// visible) and an `action` closure; the hover tracking lives here so each
/// caller doesn't repeat `@State private var isHovered`.
private struct ClickableStatusSegment<Content: View>: View {
    let systemImage: String
    let isActive: Bool
    let help: String
    let action: () -> Void
    ...
    var body: some View {
        Button(action: action) {
            StatusSegment(systemImage: systemImage, content: content)
                .background(
                    RoundedRectangle(cornerRadius: 4)
                        .fill(isHovered || isActive ? Theme.chromeHover : .clear)
                )
        }
        .buttonStyle(.plain)
...
private struct DiffToggleSegment: View {
    let gitStatus: GitStatus
    @Bindable var store: WorkspaceStore

    var body: some View {
        ClickableStatusSegment(
            systemImage: "line.3.horizontal.button.angledtop.vertical.right",
            isActive: store.rightPanel == .gitChanges,
            ...
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text("\(gitStatus.filesChanged)")
                    .foregroundStyle(Theme.chromeMuted)
                if gitStatus.insertions > 0 {
                    SignedNumber(sign: "+", value: gitStatus.insertions, color: Theme.gitInsertion)
                }
                if gitStatus.deletions > 0 {
                    SignedNumber(sign: "−", value: gitStatus.deletions, color: Theme.gitDeletion)
                }
            }
        }
    }
...
private struct SwitchableStatusSegment<Item: Hashable>: View {
    ...
    var body: some View {
        ...
        .popover(isPresented: $isSwitcherOpen, arrowEdge: .bottom) {
            HiveMenuList(width: popoverWidth, maxHeight: popoverMaxHeight) {
                if items.isEmpty {
                    HiveMenuRow(title: emptyMessage, isDisabled: true) {}
                } else {
...
/// child processes, so hive doesn't pretend to switch proxies for you.
private struct ProxyStatusSegment: View {
    ...
    var body: some View {
        ...
        .popover(isPresented: $isPopoverOpen, arrowEdge: .bottom) {
            HiveMenuList(width: 380, maxHeight: 240) {
...
/// actions rendered in the same brutalist style as the rest of hive's
...
private struct PaneContextMenu: View {
    ...
    var body: some View {
        ...
        HiveMenuList(width: 240, maxHeight: 480) {
            if !askRows.isEmpty {
            ...
            HiveMenuRow(title: "Copy", shortcut: "⌘C", isDisabled: !hasSelection) {
                isPresented = false
                NSPasteboard.general.clearContents()
                NSPasteboard.general.setString(selection, forType: .string)
            }
            HiveMenuRow(title: "Paste", shortcut: "⌘V", isDisabled: !pasteAvailable) {
                isPresented = false
            ...
            Divider()
            HiveMenuRow(title: "Select All", shortcut: "⌘A") {
                isPresented = false
                session.engine.performAction("select_all")
            }
            HiveMenuRow(title: "Clear", shortcut: "⌘K") {
                isPresented = false
...
/// constant. Keeps `HiveMenuRow`'s sibling layout consistent across the
/// app.
private struct HiveMenuList<Content: View>: View {
    let width: CGFloat
