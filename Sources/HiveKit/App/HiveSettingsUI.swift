...
/// `@Observable` mirror of the typed slice of `~/.hive/settings.json` we
...
/// have working bindings in libghostty today — hive-specific keys
...
final class HiveSettingsModel {
    ...
    static let shared = HiveSettingsModel()
    ...
    /// When true, hive launches Claude tabs with `--resume <id>` using the
    ...
    func load() {
        let parsed = HiveSettings.loadParsed() ?? [:]
    ...
    private func save() {
        var parsed = HiveSettings.loadParsed() ?? [:]
        ...
        HiveSettings.write(parsed)
        HiveShellIntegration.refreshClaudeCustomSettings(customAgents: customAgents)
    }
...
enum SettingsCategory: String, CaseIterable, Identifiable {
    case terminal, codingAgents, advanced, about
...
struct HiveSettingsView: View {
    @Bindable var model: HiveSettingsModel
    ...
    private var advancedDetail: some View {
        VStack(alignment: .leading, spacing: 0) {
            SettingsRow(label: "~/.hive/settings.json") {
    ...
    private var aboutDetail: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            SettingsRow(label: "check-for-updates") {
                BracketButton(isCheckingUpdate ? "checking…" : "check now") {
                    checkForUpdates()
                }
            ...
            SettingsRow(label: "repository") {
                BracketButton("open github") {
                    NSWorkspace.shared.open(HiveApp.repositoryURL)
                }
            }
            SettingsHairline()
            SettingsRow(label: "report-issue") {
                BracketButton("open issues") {
    ...
    private var terminalRestartCallout: some View {
        HStack(spacing: 12) {
            Text("Terminal settings apply on restart.")
                ...
                .foregroundStyle(Theme.chromeMuted)
            Spacer()
            BracketButton("restart hive", action: restartApp)
        }
        .padding(.horizontal, 28)
        .padding(.top, 22)
    }
    ...
    private func restartApp() {
        ...
        // Support/hive/socket` and the persisted workspace file. The new
        // instance reads stale state and binds to the socket that the old
        // `applicationWillTerminate` is about to delete, leaving HiveHook
        ...
        // helper inherits PID 1 once hive dies, so it keeps running after
        // our terminate.
        model.flushSave()
        let pid = ProcessInfo.processInfo.processIdentifier
        let bundle = HiveShellIntegration.quote(Bundle.main.bundlePath)
        let task = Process()
...
/// (e.g. a fresh hive install, or a new agent in a future version) are
...
/// its window closed.
@MainActor
final class HiveSettingsWindowController: NSWindowController {
    static let shared = HiveSettingsWindowController()
    private let model = HiveSettingsModel.shared
    private var storeProvider: (() -> WorkspaceStore?)?
    private var host: NSHostingController<HiveSettingsView>?
    ...
    private func buildWindowIfNeeded() {
        ...
        let view = HiveSettingsView(model: model) { [weak self] in
    ...
    /// Opens `~/.hive/settings.json` in a new hive tab via `$EDITOR`
    ...
    private func openSettingsInNewTab() {
        ...
        if !FileManager.default.fileExists(atPath: HiveSettings.url.path) {
            HiveSettings.writeDefaultTemplate()
        }
        guard let store = storeProvider?(), let workspace = store.active else {
            NSWorkspace.shared.open(HiveSettings.url)
            return
        }
        // HIVE_AGENT is auto-evaluated by the wrapper rcfile; shell expands
        // `${EDITOR:-vi}` at runtime, so the user's chosen editor wins.
        let template = AgentTemplate(
            id: "hive-settings-editor",
            title: "settings.json",
            symbol: "doc.text",
            iconAsset: nil,
            tintHex: nil,
            initialCommand: "${EDITOR:-vi} \(HiveShellIntegration.quote(HiveSettings.url.path))"
        )
