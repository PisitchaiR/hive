...
/// Reads `~/.hive/settings.json` and forwards its `terminal.*` section to
/// libghostty. JSONC-tolerant (line + block comments stripped before parse).
///
/// The schema has two layers:
///   - hive-specific keys (`agent`, `sidebar`, `tab`, …) — parsed by hive,
...
enum HiveSettings {
    ...
    /// natively (macOS 12+, hive's floor is 14). Logs but doesn't surface
    /// UI errors — hive still launches with libghostty defaults.
    static func loadParsed() -> [String: Any]? {
        ...
        guard let obj = try? JSONSerialization.jsonObject(with: data, options: [.json5Allowed]) else {
            NSLog("hive: settings.json parse failed")
            return nil
        }
    ...
    /// `ghostty_config_load_default_files` so user's hive-side keys win over
    ...
    static func apply(to config: ghostty_config_t?) {
        ...
        text.withCString { cstr in
            "hive-settings".withCString { sourceName in
...
enum HiveOnboarding {
    ...
    private static func promptGhosttyImport(from path: URL) {
        let alert = NSAlert()
        alert.messageText = "Welcome to hive"
        alert.informativeText = "We found your existing ghostty configuration. Would you like to import it into hive?\n\nYou can change settings any time via Help → Open Settings."
    ...
    /// hive owns its own copy after import so future ghostty edits won't leak
    /// in (and vice versa).
    private static func importGhosttyConfig(from path: URL) {
        ...
        HiveSettings.write(["terminal": terminal])
    }
