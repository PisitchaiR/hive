...
public final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    private var windowControllers: [HiveWindowController] = []
    ...
    /// The hive window that was key most recently. `activeStore` routes
    ...
    private weak var lastKeyController: HiveWindowController?
    ...
    public func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // surface is spawned, since `LibghosttyApp` reads `~/.hive/settings.json`
        // at process init when the first surface is created.
        HiveOnboarding.runIfNeeded()
        HiveShellIntegration.installAgentHooks()
        HiveShellIntegration.refreshClaudeCustomSettings(customAgents: HiveSettingsModel.shared.customAgents)
    ...
    private func restoreWindows() {
        ...
        // ends up frontmost — hive doesn't persist which window was key.
    }
    ...
    @discardableResult
    private func addWindow(windowId: UUID = UUID()) -> HiveWindowController {
        ...
        let controller = HiveWindowController(windowId: windowId, store: store)
    ...
    private func handleWindowWillClose(_ controller: HiveWindowController) {
        // Keep the persisted slot (restore next launch) when this is the
        // last window — closing it is effectively a quit, matching hive's
    ...
    /// When a non-hive window (Settings / Update) is key, routes to the
    /// most-recently-key hive window; nil only when no hive window exists.
    ...
    private func installMainMenu() {
        ...
        mainMenu.addItem(submenu(buildMenu(title: HiveApp.name, entries: [
            selfRow("About \(HiveApp.name)", #selector(handleAbout)),
            selfRow("Check for Updates…", #selector(handleCheckForUpdates(_:))),
            .separator,
            selfRow("Settings…", #selector(handleOpenSettings), ","),
            .separator,
            responderRow("Hide \(HiveApp.name)", #selector(NSApplication.hide(_:)), "h"),
            ...
            .separator,
            responderRow("Quit \(HiveApp.name)", #selector(NSApplication.terminate(_:)), "q"),
        ])))
    ...
    @objc private func handleNewTab() {
        ...
        let template = AgentTemplate.defaultLaunchTemplate(model: HiveSettingsModel.shared) ?? .terminal
    ...
    @objc private func handleRestartApp() {
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = "Could not restart Hive"
    ...
    private var aboutCredits: NSAttributedString {
        ...
        credits.append(NSAttributedString(
            string: HiveApp.tagline,
            attributes: attrs(body, .labelColor, tight)
        ))
        credits.append(NSAttributedString(
            string: "\nGithub ↗",
            attributes: attrs(body, .linkColor, tight, link: HiveApp.repositoryURL)
        ))
        credits.append(NSAttributedString(
            string: "\n© \(HiveApp.copyrightYear) \(HiveApp.name). All rights reserved.",
        ...
        credits.append(NSAttributedString(
            string: HiveApp.author,
            attributes: attrs(foot, .linkColor, tight, link: HiveApp.authorURL)
        ))
        return credits
    }

    @objc private func handleOpenIssues() {
        NSWorkspace.shared.open(HiveApp.issuesURL)
    }

    @objc private func handleOpenRepo() {
        NSWorkspace.shared.open(HiveApp.repositoryURL)
    }
    ...
    @objc private func handleCheckForUpdates(_ sender: NSMenuItem) {
        ...
        // HIVE_FAKE_VERSION lets us preview the "newer release" prompt without
        // mutating HiveApp.displayVersion. Launch via:
        //   open --env HIVE_FAKE_VERSION=0.11.0 /Applications/Hive.app
        let currentVersion = ProcessInfo.processInfo.environment["HIVE_FAKE_VERSION"]
            ?? HiveApp.displayVersion
        Task { @MainActor in
    ...
    @objc private func handleOpenSettings() {
        ...
        HiveSettingsWindowController.show(storeProvider: { [weak self] in self?.activeStore })
    }
