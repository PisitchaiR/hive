    ...
    var body: some View {
        ...
        .popover(isPresented: $isContextMenuOpen, arrowEdge: .trailing) {
            VStack(alignment: .leading, spacing: 0) {
                HiveMenuRow(title: "Close Workspace", shortcut: "⌘⇧W") {
                    isContextMenuOpen = false
                    onClose()
                }
                HiveMenuRow(title: "Close Other Workspaces", isDisabled: !canCloseOthers) {
                    isContextMenuOpen = false
                    onCloseOthers()
                }
                HiveMenuDivider()
                HiveMenuRow(title: "Rename Workspace…") {
                    isContextMenuOpen = false
                ...
                HiveMenuRow(title: "Duplicate Workspace") {
                    isContextMenuOpen = false
                    onDuplicate()
                }
                HiveMenuDivider()
        ...
        .popover(isPresented: $isRenameOpen, arrowEdge: .trailing) {
            HiveRenameField(placeholder: "Workspace title", text: $pendingRename) {
