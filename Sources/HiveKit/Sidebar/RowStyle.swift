...
/// One row in a hive popover menu — tab right-click, "+" agent menu, etc.
...
struct HiveMenuRow<Leading: View>: View {
    let title: String
    let shortcut: String?
    let isDisabled: Bool
    let leading: Leading
...
extension HiveMenuRow where Leading == EmptyView {
    init(
        title: String,
        shortcut: String? = nil,
        isDisabled: Bool = false,
