...
/// hive window's `WorkspaceStore`; array order is window restore order.
struct PersistedApp: Codable, Equatable {
    var windows: [PersistedWindow]
}

/// Window frame (size / position) is intentionally not persisted — hive
...
struct PersistedTab: Codable, Equatable {
    ...
    /// `decodeIfPresent` so state.json files written by pre-resume hive
    /// versions still load.
