...
/// hooks (sent by the `HiveHook` CLI): agent lifecycle events and prompt-time
...
enum HookMessage {
    ...
    /// `HiveHook` extracts it and emits this message so hive can persist
...
final class HookServer {
    ...
    /// processes and read it via `HiveHook` reaching into `Application
    /// Support`, not via this property.
    static let socketPath: String = {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let dir = support.appendingPathComponent("hive", isDirectory: true)
    ...
    func start() {
        ...
        guard fd >= 0 else {
            NSLog("hive: HookServer socket() failed")
            return
        }
        ...
        guard pathBytes.count < MemoryLayout.size(ofValue: addr.sun_path) else {
            close(fd)
            NSLog("hive: HookServer socket path too long")
            return
        }
        ...
        guard bound == 0 else {
            NSLog("hive: HookServer bind() failed errno=\(errno)")
            close(fd)
            return
        }
        guard listen(fd, 8) == 0 else {
            NSLog("hive: HookServer listen() failed errno=\(errno)")
            close(fd)
            return
        }
    ...
    private static let envKeys = [
        ...
        "NVM_BIN", "NVM_DIR", "HIVE_NODE_VERSION",
