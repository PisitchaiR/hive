...
/// home dir. The fallback prevents hive from spawning a shell at a deleted
...
final class WorkspaceStore {
    ...
    /// to `HiveSettingsModel.shared.agentOptions[id]`; tests pass a closure
    /// that returns nil so unit tests stay independent of the developer's
    /// real `~/.hive/settings.json`.
    private let optionsProvider: @MainActor (String) -> String?
    /// Reads `HiveSettingsModel.shared.resumeConversations` at spawn time;
    ...
    private struct ClosedTabState {
        ...
        let fileViewerURL: URL?
        let gitDiffInfo: GitDiffInfo?
    }
    ...
    init(
        ...
        optionsProvider: @escaping @MainActor (String) -> String? = { HiveSettingsModel.shared.agentOptions[$0] },
        resumeProvider: @escaping @MainActor () -> Bool = { HiveSettingsModel.shared.resumeConversations },
    ...
    /// Opens `url` as a file-viewer tab in the active pane. If the file is
    /// already open in any tab of that pane, focuses it instead of duplicating.
    func openFileTab(url: URL, in workspace: Workspace) {
        guard let pane = workspace.activePane ?? workspace.root.firstPane else { return }
        if let existing = pane.tabs.first(where: { $0.fileViewerURL == url }) {
            activateTab(existing, in: workspace)
            return
        }
        let session = Session(
            engine: NoOpEngine(),
        ...
        session.fileViewerURL = url
        pane.tabs.append(session)
        pane.activeTabId = session.id
        if workspace.activePaneId != pane.id { workspace.activePaneId = pane.id }
        scheduleSave()
    }

    func openDiffTab(info: GitDiffInfo, in workspace: Workspace) {
        guard let pane = workspace.activePane ?? workspace.root.firstPane else { return }
        if let existing = pane.tabs.first(where: { $0.gitDiffInfo == info }) {
            activateTab(existing, in: workspace)
            return
        }
        let session = Session(
    ...
    private func recordClosedTab(_ session: Session, pane: Pane, workspace: Workspace) {
        recentlyClosed.append(ClosedTabState(
            ...
            fileViewerURL: session.fileViewerURL,
            gitDiffInfo: session.gitDiffInfo
        ))
    ...
    func reopenLastClosedTab() -> Session? {
        ...
        if let url = state.fileViewerURL {
            openFileTab(url: url, in: workspace)
            activateWorkspace(workspace)
            return nil
        }
        if let info = state.gitDiffInfo {
            openDiffTab(info: info, in: workspace)
            activateWorkspace(workspace)
            return nil
        }
    ...
    /// across hive launches. Same-value writes are dropped so we don't
    ...
    private func spawnSession(template: AgentTemplate, initialCwd: URL, sessionId: UUID = UUID(), conversationId: String? = nil, initialPrompt: String? = nil) -> Session {
        ...
        // `refreshClaudeCustomSettings`); `hiveEnvironment` routes this
        // session's HIVE_HOOKS_PATH there.
        let claudeCustomId = template.baseAgentId == AgentTemplate.claudeCodeID && !template.extraEnv.isEmpty
            ? template.id : nil
        config.environment.merge(
            HiveShellIntegration.hiveEnvironment(for: sessionId, claudeCustomSettingsAgentId: claudeCustomId)
        ) { _, new in new }
