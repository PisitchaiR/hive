#!/usr/bin/env bash
# Build hive as a real macOS .app bundle (no Xcode project required).
#
# What this does:
#   1. swift build -c release
#   2. Assemble dist/Hive.app/Contents/{MacOS,Resources,Info.plist,PkgInfo}
#   3. Copy Hive + HiveHook binaries + the SPM resource bundle into MacOS/
...
#      Sources/HiveKit/App/AppInfo.swift's displayVersion — single source
#      of truth, no manual sync
#   5. Adhoc codesign so Gatekeeper doesn't kill it on first launch
#
# Output: dist/Hive.app — open it directly or drop into /Applications.
...
for f in .build/release/Hive .build/release/HiveHook; do
    [ -f "$f" ] || { echo "missing: $f" >&2; exit 1; }
done
[ -d ".build/release/Hive_HiveKit.bundle" ] || {
    echo "missing SPM resource bundle: .build/release/Hive_HiveKit.bundle" >&2
    exit 1
}
...
cp .build/release/Hive "${APP}/Contents/MacOS/${APP_NAME}"
cp .build/release/HiveHook "${APP}/Contents/MacOS/HiveHook"
...
cp -R .build/release/Hive_HiveKit.bundle "${APP}/Contents/Resources/"
...
RES_BUNDLE="${APP}/Contents/Resources/Hive_HiveKit.bundle"
...
# itself.
codesign --force --sign - "${APP}/Contents/Resources/Hive_HiveKit.bundle"
codesign --force --sign - "${APP}/Contents/MacOS/${APP_NAME}"
codesign --force --sign - "${APP}/Contents/MacOS/HiveHook"
