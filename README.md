...
**One-click AI agent sessions.** Claude Code · Codex · Gemini CLI · OpenCode · Amp · Cursor CLI · Copilot CLI · Grok Build · Antigravity CLI. Pick one from the `+` menu; the agent boots before your first prompt prints. Claude conversations also auto-resume across Hive restarts so closing and reopening a tab picks up where you left off.
...
**Configurable.** Settings (`⌘,`) with a sidebar layout: **Terminal** (font / cursor / size), **Agents** (drag to reorder, toggle visibility, set per-agent launch options like `--model opus`, pick a default that `+` and `⌘T` open without a popover, define your own custom agents — point a Claude Code-based one at a mirror or proxy with its own endpoint and API key), **Advanced** (open raw JSON). All overrides live in `~/.hive/settings.json` — ghostty's own `~/.config/ghostty/config` is read first and your overrides layer on top; first-launch offers to import an existing ghostty setup.

**Local by default.** No accounts, no telemetry, no cloud sync. Hive keeps its own state on your device.
...
1. Double-click `Hive.app`. You'll see the warning. Dismiss it.
2. **System Settings → Privacy & Security**, scroll to **Security**.
3. Click **Open Anyway** next to *"Hive was blocked to protect your Mac"*. Enter your password.
4. Double-click `Hive.app` again → click **Open**. Done.
</details>
...
# Open Hive.app → it now launches
sudo spctl --global-enable       # turn Gatekeeper back on
```

This is **system-wide** while disabled. Re-enable as soon as Hive launches once (the per-app whitelist persists).
</details>
...
./scripts/build-app.sh               # writes dist/Hive.app
./scripts/build-dmg.sh --build       # writes dist/Hive-vX.Y.Z.dmg
```
